# Beschreibung der Software Sysmon

## Recherche

**Was ist Sysmon? Wofür wird es verwendet?**

Sysmon (Systemmonitor) ist ein von Mark Russinovich entwickeltes Sysinternals Software-Paket. Mittlerweile gehört es Microsoft und ist ein standard Windows-Systemdienst und -Gerätetreiber, der die Systemaktivität überwacht und diese im Windows-Ereignisprotokoll protokolliert. Er enthält Informationen zu Prozesserstellungen, Netzwerkverbindungen und Änderungen an der Dateierstellungszeit. Die generierten Ereignisse lassen sich mithilfe der Windows-Ereignissammlung oder [SIEM-Agents](https://de.wikipedia.org/wiki/Security_Information_and_Event_Management) sammeln und analysieren. Damit können schädliche oder anomale Aktivitäten identifiziert werden.

> [!Info]
> Sysmon stellt weder eine Analyse der generierten Ereignisse bereit noch kann sie Angriffe verhindern oder davor schützen.

**Was ist die aktuellste Version von Sysmon?**

Die aktuellste Version von Sysmon ist [v14.13](https://learn.microsoft.com/en-us/sysinternals/downloads/sysmon)

**Was sind Beispiel-Anwendungen/Use-Cases bei welchen Sysmon helfen kann?**

Mit Sysmon lassen sich Ereignisse aufgrund verschiedener Aktivitäten generieren. Folgende Aktivitäten können dabei überwacht werden:
* Prozesserstellung (mit vollständiger Befehlszeile und Hashes)
* Prozessbeendigung
* Netzwerkverbindungen
* Verschiedene Dateiereignisse
* Laden von Treibern/Images
* Remote-Threads erstellen
* Direkter Festplattenzugriff
* Prozessspeicherzugriff
* Registry-Zugriff (erstellen, ändern, löschen)
* Benannte Pipes
* WMI-Ereignisse

Installiert man daher Sysmon auf verschiedene Geräte z.B. in einem Unternehmen, können diese jeweils Ereignisse generieren. Anhand dieser Ereignisse sollen die Systeme überwacht werden. Diese Daten können dann über einen WEF-Server (Windows Event Forwarding) gesammelt und an eine Software wie beispielsweise Elastic Beats zur Verarbeitung und Bereitstellung weitergegeben werden. So ungefähr ist es auch bei unserem LAB aufgebaut:

![Sysmon, WEF und ELK Schema](images/Sysmon_WEF_ELK.png)

## Installation

### Manuelle Installation

Zuerst muss Sysmon über folgenden Link heruntergeladen werden: [Download Sysmon](https://learn.microsoft.com/en-us/sysinternals/downloads/sysmon)

Danach entpackt man die ZIP-Datei und öffnet im cmd den Pfad der Installationsdatei. Die folgenden Befehle und Optionen zeigen, wie Sysmon installiert werden kann.

* Installieren: sysmon -i \[configfile\]
* Aktualisieren der Konfiguration: sysmon -c \[configfile\]
* Ereignismanifest installieren: sysmon -m
* Druckschema: sysmon -s
* Deinstallieren: sysmon -u \[force\]
...
Installieren von Sysmon mit einer Konfigurationsdatei:
```
...
Konfigurieren eines aktiven Sysmon mit einer Konfigurationsdatei:
```
...
Die Installation von Sysmon wurde auf den Windows Servern/Clients des LABs automatisiert durchgeführt. Dazu wurde bei der Erstellung der VM mit Vagrant das Skript *install-sysinternals.ps1* durchgeführt. Dieses Skript führt folgende Aufgaben aus:

1. Erstellung der Verzeichnisse *C:\Tools\Sysinternals* und *C:\ProgramData\Sysmon*:
...
2. Download und Installation der folgenden Sysinternals Tools:
   * Autoruns64
   * Procmon
   * PsExec64
   * procexp64
...
3. Download der Sysmon-Config von Olaf Hartong:
...
4. Installation und Starten von Sysmon mit der Config-Datei:
...
Die Sysmon-Konfiguration ist in folgender Datei gespeichert: *C:\ProgramData\Sysmon\sysmonConfig.xml*

Die Struktur der Datei wird nun erklärt. Die Regeln sind in erster Linie in die verschiedenen EventIDs gruppiert:
...
| 10 | ProcessAccess |
| 11 | FileCreate |
| 12, 13, 14 | RegObject added/deleted, RegValue Set, RegObject Renamed |
| 15 | FileStream Created |
| 17, 18 | PipeEvent. Log Named pipe created & Named pipe connected |
| 19, 20, 21 | WmiEvent. Log all WmiEventFilter, WmiEventConsumer, WmiEventConsumerToFilter activity |
| 22 | DNS Queries and their results |
| 23 | File Delete and overwrite events which saves a copy to the archivedir |
| 24 | Clipboard change events, only captures text, not files |
| 25 | Process tampering events |
| 26 | File Delete and overwrite events |

Für jeder dieser EventIDs gibt es einen Abschnitt mit "Includes" und "Excludes". Diese enthalten jeweils die Regeln, welcher inkludiert bzw. exkludiert wurden. Sie werden mit ```<Event onmatch="include">``` und ```<Event onmatch="exclude">``` angegeben.

Schliesslich werden noch die Images definiert, welche bestimmen, auf was sich die Regeln beziehen. Hierbei können folgende Bedingungen (condition) gesetzt werden:
* is
* is not
* contains
* excludes
...
Hier werden die Include und Exclude-Regeln der Event ID 2 definiert.

```xml
<!-- Event ID 2 == File Creation Time - Includes -->
<RuleGroup groupRelation="or">
    <FileCreateTime onmatch="include"> <!-- Regeln für das Event: "FileCreationTime" werden inkludiert -->
    <Image name="technique_id=T1070.006,technique_name=Timestomp" condition="begin with">C:\Temp</Image> <!-- Regeln bezieht sich auf Dateien, dessen Pfad mit "C:\Temp" beginnen -->
...
<RuleGroup groupRelation="or">
    <FileCreateTime onmatch="exclude"> <!-- Regeln für das Event: "FileCreationTime" werden exkludiert -->
    <Image condition="end with">AppData\Local\Google\Chrome\Application\chrome.exe</Image> <!-- Regeln bezieht sich auf Dateien, dessen Pfad auf "AppData\Local\Google\Chrome\Application\chrome.exe" endet -->
...
Die gesamte sysmonConfig.xml Konfigurationsdatei gäbe es noch hier:

<details>
  <summary>Hier klicken zum anzeigen: sysmonConfig.xml</summary>
...
Der folgende Screenshot soll zeigen, wie ein generiertes Ereignis in der Ereignisanzeige (unter *Applications and Services Logs\Microsoft\Windows\Sysmon\Operational*) erscheint. Zu erkennen sind:
* EventID -> Die vordefinierte Event-Nummer
* TaskCategory -> Die Beschreibung bezieht sich auf das Event
* RuleName -> Die Nummer bezieht sich auf das MITRE ATT&CK Framework 
* TargetFilename -> Der Pfad bezieht sich auf die Datei / das Verzeichnis, welches das Ereignis ausgelöst hat.

![Sysmon Ereignisanzeige](images/Sysmon_Ereignisanzeige.png)

### Hintergründe der Konfiguration

Die folgenden Informationen sind aus dem Blog von Olaf Hartong entnommen:
...
Es gibt bereits viele verschiedene Konfigurationen zu Sysmon im Internet zu finden. Olaf Hartong wollte allerdings eine modulare Konfiguration schaffen, welche einfach zu warten ist. Damit soll es für ihn, seine Kunden und anderen Nutzern eine Grundlage bieten, den individuellen Anforderungen entsprechend, einzelne Teile der Konfiguration hinzuzufügen oder zu entfernen. Damit soll unter anderem auch die Übersicht und die Performance verbessert werden, indem in einer anderen Umgebung nicht benötigte Komponente keinen "Lärm/Rauschen" (Noise) verursachen. Diese Anpassungen ist immer ein Kompromiss zwischen Übersichtlichkeit und Lärm. Am Ende muss jeder selbst entscheiden, wo man das Risiko eingehen möchte, etwas wegzulassen.

Zudem wollte er möglichst viele Punkte des MITRE ATT&CK Frameworks abdecken. MITRE ATT&CK (Adversarial Tactics, Techniques and Common Knowledge) ist ein Framework, eine Reihe von Datenmatrizen und ein Bewertungstool. Dieses Tool wurde von der Firma MITRE entwickelt, um Organisationen dabei zu helfen, ihre Sicherheitsbereitschaft zu verstehen und ihre Schwachstellen aufzudecken. Ein Link zur ATT&CK Matrix ist [hier](https://attack.mitre.org/).

Für seine Konfiguration hat er schliesslich eine Grundkonfiguration verwendet, welches im wesentlichen eine leere Vorlage ist und von jedem verwendet werden kann. Die Struktur hat er dann zuerst nach EventID gruppiert und diese nochmals in "Include" und "Exclude" Blöcke unterteilt. So ist es leicht nachvollziehbar, für welches Event, welche Regel inkludiert bzw. exkludiert wird. Auch das Anpassen, Hinzufügen und Entfernen von Regeln soll somit erleichtert werden.

> [!Tip]
> Die ganze Konfiguration ist auch auf GitHub zu finden: [olafhartong/sysmon-modular](https://github.com/olafhartong/sysmon-modular)
